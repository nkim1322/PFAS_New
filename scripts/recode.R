...
# - Any well with concentration below  median/sqrt(2) DL for compound is considered a 
# nondetect with value recoded to median/sqrt(2) for purposes of summary statistics
# 
# PFOA: median = 2, cutoff = 1.414
# PFHXA: median = 4.5, cutoff = 3.182 
# PFPEA: median = 4.5, cutoff = 3.182 - note: we end up not needing to throw out any PFPEA!
# PFHPA: median = 2, cutoff = 1.414 
# PFOS: median = 4, cutoff = 2.828
...
recode <- function(PFAS, data){
  ...
  cutoff <- round(med/sqrt(2), digits = 3)
  data[[qualifier_name]][data[[qualifier_name]] == '' & data[[results_name]] < cutoff] <- '<'
  data[[results_name]][data[[results_name]] < cutoff] <- cutoff
  return(data)
}
...
# PFHP: Remove wells that are NDs with DLs of over 10 ---------------------
